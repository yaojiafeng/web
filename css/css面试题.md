...
### 10. 请解释一下CSS3的flex布局,以及适用场景？
	...
	flex布局又名为‘弹性布局’，主要是通过设置的属性值自动调整元素的宽度和高度值，在容器空间充裕的情况下，使其填满容器的剩余空间；又或者在容器空间不足的情况下，收缩元素的高度和宽度，使其不会溢出容器。使用flex布局，可以灵活的控制元素在容器内的展示。尤其是对于移动端小屏幕来说，flex基本已成了主流的布局方式。
